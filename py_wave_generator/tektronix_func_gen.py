...
# _VISA_ADDRESS = "USB0::0x0699::0x0353::1725304::INSTR"
_VISA_ADDRESS = "USB0::0x0699::0x035C::C012051::INSTR"
...
class FuncGen:
    ...
    def _initialise_model_properties(self):
        ...
        elif np.any(["AFG31152" in a for a in [self._model, self._override_compat]]):
            self.instrument_limits = {
                "frequency lims": ({"min": 1e-6, "max": 25e6}, "Hz"),
                "voltage lims": (
                    {"50ohm": {"min": -2.5, "max": 2.5}, "highZ": {"min": -5, "max": 5}},
                    "V",
                ),
                "amplitude lims": (
                    {
                        "50ohm": {"min": 0.0001, "max": 2.5},
                        "highZ": {"min": 0.0001, "max": 5},
                    },
                    "Vpp",
                ),
            }
...
def sin_control(address: str, wFre, wAmp, wOff):
    ...
    with FuncGen(address) as fgen:
        ...
        fgen.ch1.set_frequency(wFre, unit="Hz")
        fgen.ch1.set_offset(wOff)
        fgen.ch1.set_amplitude(wAmp)
        fgen.ch1.set_output("ON")
        fgen.ch2.set_output("OFF")
        # Alternatively fgen.ch1.print_settings() to show from one channel only
        fgen.print_settings()

def dc_control(address: str, wFre, wAmp, wOff):
    """Example showing how to connect, and the most basic control of the
    instrument parameters"""
    print("\n\n", example_basic_control.__doc__)
    with FuncGen(address) as fgen:
        fgen.ch1.set_function("DC")
        # fgen.ch1.set_frequency(0, unit="Hz")
        fgen.ch1.set_offset(wOff)
        fgen.ch1.set_amplitude(wAmp)
        fgen.ch1.set_output("ON")
        fgen.ch2.set_output("OFF")
...
if __name__ == "__main__":
    # # 从命令行获取操作符和两个数值
    operator = sys.argv[1]
    WaveFrequency = float(sys.argv[2])
    WaveAmplitude = float(sys.argv[3])
    WaveOffset = float(sys.argv[4])
    ...
    if operator == "SIN":
        sin_control(_VISA_ADDRESS, WaveFrequency, WaveAmplitude, WaveOffset)
    elif operator == "DC":
        dc_control(_VISA_ADDRESS, WaveFrequency, WaveAmplitude, WaveOffset)
    elif operator == "+":
        result = WaveFrequency + WaveAmplitude
        print(result)
    elif operator == "-":
        result = WaveFrequency - WaveAmplitude
        print(result)
